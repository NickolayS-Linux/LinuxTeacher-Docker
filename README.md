...
<img width="736" height="146" alt="image" src="https://github.com/user-attachments/assets/54a76732-8eaf-4fa5-aaf9-683178efd4ff" />
...
Собрать ядро в докере можно. 

Но при этом нужно подготовить скрипты для выполнения в докере. 
Подготовить загрузочный образ (img). 
Созданный файл образа для флеш-накопителя (linux-{version}-busybox-{версион}-disk.img) можно записать на флеш-накопитель при помощи утилит, которые позволяют это сделать, например 

(например, Win32DiskImager) 
...
**Слоистая структура.** Образ состоит из нескольких слоёв файловой системы, каждый из которых представляет собой результат выполнения одной из инструкций в Dockerfile 
